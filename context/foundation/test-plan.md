...
| #2 | After a "successful" save the row is actually present in the DB with all required fields; a DB-rejected write (Supabase `error !== null`) redirects to the form page with `?error=` in the URL — not to `/fights`. | "Redirect to `/fights` (no `?error=`) ⇒ the fight persisted." (All fight mutations return HTTP 302; HTTP status alone cannot distinguish success from error.) | All fight mutations return HTTP 302 redirects, never 200. DB rejection = redirect to form page with `?error=`. The persisted state must be verified by querying the fights row directly. The write entry point is a single `.insert()` — no partial-write risk. | integration (direct Supabase operations against seeded DB — no running server needed) | Asserting the HTTP status or redirect URL instead of asserting the persisted DB row's field values. |
